...
export default function EnglishModulesPage({ navigateTo }: EnglishModulesPageProps) {
  ...
  return (
    <div className="min-h-screen bg-[#050505] text-white font-sans selection:bg-brand-green selection:text-black overflow-x-hidden">
      ...
      <motion.nav initial={{ y: -100 }} animate={{ y: 0 }} transition={{ duration: 0.8, ease: [0.16, 1, 0.3, 1] }} className="sticky top-0 z-50 border-b border-white/5 bg-[#050505]/80 backdrop-blur-xl">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center justify-between h-20">
            <motion.button onClick={() => navigateTo('home')} whileHover={{ x: -5 }} className="flex items-center gap-2 text-gray-500 hover:text-brand-green transition-colors group">
            ...
            <div className="flex items-center gap-3 group cursor-pointer" onClick={() => navigateTo('home')}>
              ...
              <span className="text-xl font-display font-bold tracking-tight">GuiEnglish</span>
            </div>
            <div className="hidden sm:block">
              <button onClick={() => navigateTo('login')} className="px-5 py-2 rounded-full border border-white/10 hover:border-brand-green/50 hover:bg-brand-green/5 transition-all text-sm font-medium">Entrar</button>
            </div>
          </div>
        </div>
      </motion.nav>
      ...
      <section className="relative py-20 px-4 overflow-hidden">
        <div className="max-w-4xl mx-auto text-center relative z-10">
          <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8 }}>
            ...
            <h1 className="text-5xl sm:text-7xl font-display font-bold mb-6 leading-tight">
              Escolha seu <br /><span className="gradient-text">ritmo de sucesso</span>
            </h1>
        ...
        <div className="absolute inset-0 opacity-5 pointer-events-none" style={{ backgroundImage: 'linear-gradient(rgba(16, 185, 129, 0.1) 1px, transparent 1px), linear-gradient(90deg, rgba(16, 185, 129, 0.1) 1px, transparent 1px)', backgroundSize: '40px 40px' }} />
      </section>

      <section className="pb-32 px-4 relative">
        <div className="max-w-7xl mx-auto">
          <motion.div initial="initial" animate="animate" variants={{ animate: { transition: { staggerChildren: 0.1 } } }} className="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
      ...
      <footer className="py-12 border-t border-white/5 bg-[#0a0a0a]">
        <div className="max-w-7xl mx-auto px-4 text-center">
          <p className="text-gray-500 text-sm">Dúvidas? Entre em contato com nosso suporte especializado.</p>
          <div className="mt-6 flex justify-center gap-6">
            {['Termos', 'Privacidade', 'Suporte'].map(item => (
              <a key={item} href="#" className="text-xs text-gray-600 hover:text-brand-green transition-colors">{item}</a>
            ))}
          </div>
        </div>
      </footer>
