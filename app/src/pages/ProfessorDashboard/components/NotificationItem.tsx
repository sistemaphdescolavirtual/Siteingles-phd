          ...
          <div className="flex flex-wrap items-center justify-between gap-4">
            ...
            {notification.type === 'autorizacao' && notification.data?.alunoId && (
              <div className="flex gap-3">
                <Button onClick={() => onAprovar(notification.data.alunoId)} className="bg-brand-neon hover:bg-brand-lime text-black font-bold rounded-xl px-5 h-10 cursor-pointer">
                  <Check className="w-4 h-4 mr-2" /> Aprovar
                </Button>
                <Button onClick={() => onRejeitar(notification.data.alunoId)} variant="outline" className="border-red-500/30 text-red-400 hover:bg-red-500/10 rounded-xl px-5 h-10 cursor-pointer">
